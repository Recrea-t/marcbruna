...
const Logo = (props) => {
	...
	if (!url) {
		return <Img className="client-logo" fluid={logo.childImageSharp.fluid} alt={name} objectFit="contain" imgStyle={imgStyle} />
	}

	return (
		<a className="client-logo" href={url} title={name} target="_blank" rel="noreferrer">
			<Img fluid={logo.childImageSharp.fluid} alt={name} objectFit="contain" imgStyle={imgStyle} />
		</a>
	)
}
