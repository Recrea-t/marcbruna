  ...
  return (
    <div className="servei">
      ...
      <div className="d-flex flex-column">
        <Link to={link} className="w-100 btn btn-primary mt-auto" title={name}>
          {name}
        </Link>
      </div>
    </div>
...
const Products = () => {
  ...
  const products = menuLinks.find((item) => item.name === "Serveis")
    .submenuLinks;

  return (
    <div className="serveis">
      {products.map((item, index) => (
        <Product key={index} index={index} product={item} />
      ))}
    </div>
  );
};
