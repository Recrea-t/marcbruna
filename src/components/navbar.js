...
import finstral from '../images/Logo_Finstral.svg'
		...
		<Navbar bg="primary" expand="lg" fixed="top">
			<Container>
				<Navbar.Text className="p-0">
					<a href={`phone:${organization.phone.number}`}>
						<FontAwesomeIcon icon="phone" className="me-1" />
					</a>
					<a href={`mailto:${organization.email}`}>{organization.email}</a> | Distribuidor de <img src={finstral} alt="Finstral" className="logo-finstral" />
				</Navbar.Text>
				<Nav className="flex-row ms-auto p-0 mb-lg-0">
					<Nav.Link className="ps-2 ps-lg-0" href={`https://www.instagram.com/${social.instagram}`} target="_blank" title="Segueix-nos a Instagram">
						<FontAwesomeIcon icon={['fab', 'instagram']} />
					</Nav.Link>
					<Nav.Link className="ps-2 ps-lg-0" href={`https://www.facebook.com/${social.fbAppId}`} target="_blank" title="Segueix-nos a Facebook">
						<FontAwesomeIcon icon={['fab', 'facebook']} />
					</Nav.Link>
					<Nav.Link className="ps-2 ps-lg-0" href={`https://www.pinterest.com/${social.pinterest}`} target="_blank" title="Segueix-nos a Pinterest">
						...
						<div className={`navbar-collapse offcanvas-collapse px-0 ${this.state.navBarActiveClass}`}>
							<Container className="pt-3 pt-lg-0">
								<Navbar.Toggle
									...
									onClick={() => this.toggleHamburger()}
								/>
								<Nav className="me-lg-auto mb-0 fw-bold float-end" activeKey={this.props.pathname}>
